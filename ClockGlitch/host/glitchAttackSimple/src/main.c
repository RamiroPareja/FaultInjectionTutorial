...
int main(void)
{
	...
	IP_GPIO_WriteDirBit(LPC_GPIO,2,1);		// Reset
	IP_GPIO_WriteDirBit(LPC_GPIO,19,1);		// Synch
	IP_GPIO_WriteDirBit(LPC_GPIO2,11,0);	// Bottoms
	IP_GPIO_WriteDirBit(LPC_GPIO2,12,0);

	IP_GPIO_WritePortBit(LPC_GPIO,2,9);		// Reset hold to 1
	...
	asm("LOOP_WAITING_BUTTON1:");	// This loop generates a 15 MHz clock signal while PB1 is not pressed
	...
	asm("str	r1, [r0,#28]");		// This instruction is to wait just one instruction cycle.
